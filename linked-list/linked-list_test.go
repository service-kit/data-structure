...
func Test_SingleCircularLinkedList(t *testing.T) {
	ll := new(SingleCircularLinkedList)
	ll.Init()
	for i := 1; i <= 100; i++ {
		ll.Insert(i, i)
	}
...
func Test_DoubleCircularLinkedList(t *testing.T) {
	d := new(DoubleCircularLinkedList)
	d.Init()
	for i := 0; i < 100; i++ {
		d.PushBack(i)
	}
